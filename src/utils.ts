...
export const getType = (value: any = undefined): string => {
  ...
  return (typeRegex.exec(Object.prototype.toString.call(value)) ?? [
    '',
    'Undefined'
  ])[1]
}
 ...
 */
export const isEqual = <TValue, TOther>(
  value: TValue,
  other: TOther
): boolean => {
  ...
  for (const key of Object.keys(value)) {
    if (typeof Reflect.get(value as unknown as object, key) === 'object') {
      if (
        !isEqual(
          Reflect.get(value as unknown as object, key),
          Reflect.get(other as unknown as object, key)
        )
      ) {
        return false
      }
